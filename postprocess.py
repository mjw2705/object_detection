...
class Postprocess():
    ...
    def forward(self, raw_yolo_output):
        ...
        for raw_yolo_out in raw_yolo_output:
            ...
            objectness.append(raw_yolo_out[1].view(batch_size, -1, 1))
            class_prob.append(raw_yolo_out[2].view(batch_size, -1, num_classes))

        boxes = xywh_to_x1x2y1y2(torch.cat(boxes, dim=1))
        objectness = torch.cat(objectness, dim=1)
        class_prob = torch.cat(class_prob, dim=1)

        return self.batch_non_maximum_suppression(boxes, objectness, class_prob)
    ...
    def batch_non_maximum_suppression(self, boxes, scores, classes):
        def single_batch_nms(candidate_boxes):
            ...
            if count > 0:
                ...
                updates = torch.cat((torch.stack(updates), torch.stack(count_update)), dim=0)
                # dim=0으로 outputs의 indices위치에 updates값 넣기
                outputs = outputs.scatter_(0, indices, updates)
